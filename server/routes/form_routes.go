...
func RegisterFormRoutes(e *core.ServeEvent) {
	...
	e.Router.PATCH("/api/forms/project/task/updatebulk", func(e *core.RequestEvent) error {
		return formfunctions.UpdateTasksBulk(e)
	}).Bind(apis.RequireAuth()) // Require authentication, remove if not needed

	e.Router.GET("/api/forms/project/members", func(e *core.RequestEvent) error {
	...
	e.Router.POST("/api/forms/project/members/update", func(e *core.RequestEvent) error {
		return formfunctions.UpdateUserRoleInProject(e)
	}).Bind(apis.RequireAuth()) // Require authentication, remove if not needed

	e.Router.GET("/api/forms/project/task/mails", func(e *core.RequestEvent) error {
		return formfunctions.GetEmailsForTask(e)
	}).Bind(apis.RequireAuth()) // Require authentication, remove if not needed
